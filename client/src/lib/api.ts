...
class ApiService {
  ...
  async login(email: string, password: string): Promise<{ user: User; token: string }> {
    if (this.isMockMode) {
      const credential = DUMMY_CREDENTIALS.find((cred) => cred.email === email && cred.password === password)
      if (credential) {
        return {
  ...
  async getDestinations(params?: { search?: string; category?: string; featured?: boolean }): Promise<Destination[]> {
  ...
  // Reviews endpoints
  async getReviews(hotelId?: string, destinationId?: string): Promise<Review[]> {
    const searchParams = new URLSearchParams()
  ...
  async createReview(review: Omit<Review, "id" | "createdAt" | "user">): Promise<Review> {
